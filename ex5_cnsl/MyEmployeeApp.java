...
public class MyEmployeeApp {
    ...
    {
        String nname,tname;
		char []password=new char[]{'G','o','p','i','9','7'};
        nname="Vinothgopi";
        try
        {
            Console cnsl=System.console();
            tname=cnsl.readLine("Enter user name : ");
            char []tpassword=cnsl.readPassword("Enter password : ");
			
			boolean flag=true;
			for(int i=0;i<password.length;i++)
			{
				if(tpassword[i]!=password[i])
				{
						flag=false;
						break;
				}
			}
            if(tname.equals(nname) && flag)
            {
                Employee emp1 = null;
